...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
    ...
    {{-- Cancelled (click -> cancelled page) --}}
...
<div class="rounded-3xl bg-white/5 border border-white/10 shadow-xl overflow-hidden">
    <div class="p-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            @php
    $tabs = ['all'=>'All','approved'=>'Approved','pending'=>'Pending','cancelled'=>'Cancelled'];
@endphp
        ...
        <form method="GET" action="{{ route('admin.reservations.index') }}" class="w-full max-w-md">
            ...
            <div class="relative">
                ...
                <input name="q" value="{{ $q }}"
                       placeholder="Search by room or organizer..."
...
@if($r->status === 'pending')
    {{-- APPROVE --}}
    <form method="POST" action="{{ route('admin.reservations.approve', $r->reserve_id) }}" class="inline">
        @csrf
        @method('PATCH')
        <button type="submit"
    ...
    {{-- REJECT --}}
    <form method="POST" action="{{ route('admin.reservations.reject', $r->reserve_id) }}" class="inline">
        @csrf
        @method('PATCH')
